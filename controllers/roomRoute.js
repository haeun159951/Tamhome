...
const { authenticatedForAdmin, authenticated } = require("../middleware/userAuth");
...
router.get("/roomList", function (req, res) {
    roomModel.find({}).exec().then(rooms => {  
        res.render('roomList', { 
            rooms, 
...
router.get("/roomList/:location",  function (req, res) {
    cityFilter = req.params.location === "all" ? {}: {location: req.params.location}
    roomModel.find(cityFilter).exec().then(rooms => {
        res.render('roomList', { 
            rooms, 
            fname: req.session.user ? req.session.user.fname: "",
            isAdmin: req.session.user && req.session.user.isAdmin
...
function editOrSave(req, res, edit, callback) {
    ...
    else
    {
        ...
        const room =
        {
            ...
            pic : req.file && req.file.path.substring(req.file.path.indexOf(process.platform.substring(0, 3) === "win" ? "\\": "/"))
        };
...
router.post("/edit", authenticatedForAdmin, upload.single("pic"), (req, res) => {
    editOrSave(req, res, true, (room) => {
        console.log(room)
        roomModel.updateOne({_id: room.id},
            { $set:{
                title:room.title,
                price:room.price,
                description : room.description,
...
{
    roomModel.deleteOne({_id:req.params.roomid})
    .then((err) =>{
        console.error(`deleted in the database`)
        res.redirect("/room/roomList")
    })
    .catch(err=>console.error(`Error with deleting data from the database : ${err}`));
});

//  room-detail database 
router.get("/detail/:roomid", (req, res) => {
    let user = req.session.user
    roomModel.findById({_id:req.params.roomid})
        .exec()
        .then((room) => res.render('detail', 
            { 
                room: room, 
